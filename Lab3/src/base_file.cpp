...
BaseFile::BaseFile(BaseFile&& other) {
    file       = other.file;
    other.file = nullptr;   // other no longer owns the file
    cout << "[BaseFile] move ctor, this=" << this << "\n";
}

// ── move assignment ───────────────────────────────────────────────────────────
BaseFile& BaseFile::operator=(BaseFile&& other) {
    if (this == &other) return *this;
    if (file) fclose(file);   // close our current file first
    file       = other.file;
    other.file = nullptr;
    cout << "[BaseFile] move assign, this=" << this << "\n";
    return *this;
}
